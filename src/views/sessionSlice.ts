...
export const gameSlice = createSlice({
  ...
  reducers: {
    setActiveGame: (state, action) => {
      const game = state.byId.find((g) => g.name === action.payload);
    ...
    addGame: (state, action) => {
      const gameIndex = state.byId.findIndex((g) => {
        return g.name === action.payload.name
      });
      if (gameIndex !== -1) state.byId.splice(gameIndex, 1, action.payload);
      else state.byId.push(action.payload)
    },
    deleteGame: (state, action) => {
      const gameIndex = state.byId.findIndex((g) => g.name === action.payload);
      state.byId.splice(gameIndex, 1);
    },
  }
})
